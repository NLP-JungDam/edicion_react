...
const JobList = () => {
    ...
    return (
        <div className={styles.jobList}>
                    ...
                    <React.Fragment key={job._id}>
                        <li className={styles.jobContainer}>
                            <div className={styles.checkboxWrapper}>
                                <label>
                                    <input type="checkbox" checked={selectedJobs.includes(job._id)} onChange={() => handleCheckboxChange(job._id)} />
                                    <span className={styles.customCheckbox}></span>
                                </label>
                                <span>{job.businessName}</span>
                            </div>
                            <span onClick={() => toggleJobDetail(index)}>{selectedJobIndex === index ? '▲' : '▼'}</span>
                        </li>
            ...
            <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} message="이력서가 제출되었어요!" buttonText="제출 내역 보러 가기" onConfirm={() => navigate('/user/info')} />
        </div>
    )
}
